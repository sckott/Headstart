...
popup.drawDropdown = function() {
    ...
    $.each(main_canvas.bubbles, function(index, entry) {
        ...
        $("#dataset_dropdown").append('<li><a  id="' + index + '" href="#">' + entry.title + '</a></li>');
    })

    $('#dataset_dropdown').on("click", "a", function(){
      update_file($(this).attr("id"));
    });
    ...
    //     update_file(datasets.selectedIndex + 1);
    // })
}

update_file = function(selected_file_number) {
...
popup.drawInfoLinkWithTitle = function(title) {
    ...
    var info = d3.select("#subdiscipline_title h1")
        ...
        .html(main_canvas.subdiscipline_title);
}
